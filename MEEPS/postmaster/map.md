...
> **⚠ PENDING CUTOVER (2026-08-06, Keemin-approved, awaiting the trigger): the 07:00/19:00 slot becomes the MAIL ROUND when the Registrar takes the door round.** Slugs then: `postmaster-mail-am/pm` replace `postmaster-door-am/pm`. **The count stays six.** Trigger = the Registrar's first live door fire; until then the door crons stand and the mail round's owed-replies check runs as the first step of the town round. Design + reasoning: `MEEPS/SKILLS/postmaster-mail-round.md § Cron cutover`. **On any wake before the trigger, re-heal the SIX IN THE TABLE BELOW as they stand.**
...
| 07:00 daily | `0 7 * * *` | **NOW:** `/postmaster-door-round — cron-fired PR queue + join admission + merge law + welcomes; follow MEEPS/SKILLS/postmaster-door-round.md end-to-end, the skill is source of truth.`<br>**ON CUTOVER →** `/postmaster-mail-round — cron-fired office correspondence: the owed-replies check, triage, and the letters the office owes; follow MEEPS/SKILLS/postmaster-mail-round.md end-to-end, the skill is source of truth.` |
